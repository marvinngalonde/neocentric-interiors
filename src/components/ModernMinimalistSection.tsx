...
const ModernMinimalistSection = () => {
  ...
  return (
    <section
      ref={ref}
      className={`w-full h-[40rem] overflow-hidden transition-all duration-1000 ease-out ${
        isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
      }`}
    >
      <div className="flex animate-slide-left h-[40rem] w-[50rem]">
        {/* First set of images */}
        {images.map((image, index) => (
          <div
            key={`first-${index}`}
            className="flex-shrink-0 w-full h-[40rem]"
          >
            <img
              src={image}
              alt={`Slide ${index + 1}`}
